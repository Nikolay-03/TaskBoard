<script lang="ts">
    ...
    import {type ITask, useCreateTask, useUpdateTask} from '$api/task';
    ...
    let title = $state<string>(defaultValues?.title ?? '');
    let description = $state<string>(defaultValues?.description ?? '');
    let dueDate = $state<CalendarDate>(defaultValues?.dueDate ? parseDate(defaultValues.dueDate) : today(getLocalTimeZone()));
    let labels = $state<number[]>(defaultValues?.labels.map(l => l.id) ?? []);
    let participants = $state<number[]>(defaultValues?.participants.map(p => p.id) ?? []);
    let assignees = $state<number[]>(defaultValues?.assignees.map(a => a.id) ?? []);
